...
export default class CivilProducer extends AbstractProducer {
    ...
    ///////////////////////////////////////////////////////////////////////
    //helps find the latest date
    ///////////////////////////////////////////////////////////////////////
    async getTheLatestDate(county: String, propertyState: String, fullStateName: String) {

        let dateToReturn: any = new Date();
        dateToReturn.setMonth(dateToReturn.getMonth() - 1);
        dateToReturn = this.getFormattedDate(dateToReturn);

        const lastRecord = await db.models.PublicRecordLineItem.findOne({ County: county, 'Property State': propertyState }, null, { sort: { createdAt: -1 } });
        //no record exist
        if (!lastRecord) {
            return dateToReturn;
        }
        //if there is records, try to find the one with the latest date
        const latestDates = [];
        const practiceTypes = ['preforeclosure', 'eviction', 'marriage', 'hoa-lien', 'personal-injury', 'debt', 'other-civil']
        for (const practiceType of practiceTypes) {
            //get the product id for the current practiceType
            const productName = `/${this.publicRecordProducer.state.toLowerCase()}/${this.publicRecordProducer.county}/${practiceType}`;
            const prod = await db.models.Product.findOne({ name: productName }).exec();
            const currentProductId = prod._id;
            const lastRecordInTheCurrentPracticeType = await db.models.PublicRecordLineItem.findOne({ County: county, 'Property State': propertyState, 'productId': currentProductId }, null, { sort: { createdAt: -1 } });
            if (lastRecordInTheCurrentPracticeType && lastRecordInTheCurrentPracticeType.fillingDate) {
                latestDates.push(lastRecordInTheCurrentPracticeType.fillingDate);
            }
        }
        // get the latest date from the array
        if (latestDates.length != 0) {
            let formatedDates: any = latestDates.map(function (dateElem) { return new Date(dateElem); })
            let latestDate: any = new Date(Math.max.apply(null, formatedDates));
            dateToReturn = this.getFormattedDate(latestDate);
        }
        return dateToReturn;
    }
    ...
    ///////////////////////////////////////////////////////////////////////
    async checkIfCaseWasAlreadyProcessed(county: string, fullName: string, fillingDate: string, practiceType: string) {
        const recordExist = await db.models.PublicRecordLineItem.findOne({ County: county, 'Full Name': fullName, fillingDate: fillingDate, practiceType: practiceType });
        if (recordExist) {
            return true;
        }
        else {
            //in case date was null
            const recordWithoutDate = await db.models.PublicRecordLineItem.findOne({ County: county, 'Full Name': fullName, practiceType: practiceType });
            if (recordWithoutDate) {
                return true;
            }
        }
        return false;
