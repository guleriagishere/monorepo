...
export default class CivilProducer extends AbstractProducer {
    ...
    async saveRecord(fullName: string, fillingDate: string, documentType: string, caseId: string) {
        ...
        if (String(normalizedFillingDate) === "Invalid Date") {
            normalizedFillingDate = null;
        }
        else {
            fillingDate = this.getFormattedDate(normalizedFillingDate);
        ...
        //save record
        try {
            let finalDoc = new db.models.PublicRecordLineItem(docToSave);
