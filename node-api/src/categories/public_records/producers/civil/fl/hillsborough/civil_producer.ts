...
export default class CivilProducer extends AbstractProducer {
  ...
  async checkFillingDateInDB(county: string, civilDataFillingDate: string) {
    const data = await db.models.PublicRecordLineItem.findOne({ County: county, fillingDate: civilDataFillingDate });
    if (data) {
      return true;
    }
    return false;
  ...
  async parseJsonAndSave(jsonResData: any, civilDataFillingDate: string) {
    let count = 0;
    const checkDuplicates = await this.checkFillingDateInDB('Hillsborough', civilDataFillingDate);
    if (checkDuplicates) {
      return false;
    }
