...
export default class CivilProducer extends AbstractProducer {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        ///////////////////////////////////////////////////////////////////////
        async function checkIfCaseWasAlreadyProcessed(county: string, caseUniqueId: string) {
            const data = await db.models.PublicRecordLineItem.findOne({ County: county, caseUniqueId: caseUniqueId });
            if (data) {
                return true;
            }
            return false;
        ...
        async function scrapRecordsPaths(recordsPaths: any) {
                ...
                for (const recordsPath of recordsPaths) {
                    ...
                    if (await checkIfCaseWasAlreadyProcessed('pinellas', caseId)) {
                    ...
                    try {
                        let ownerObj: any = {...docToSave};
                        if(ownerObj.hasOwnProperty('Property Address')){
                            delete ownerObj["Property Address"];
                        }
                        if(ownerObj.hasOwnProperty('Property City')){
                            delete ownerObj["Property City"];
                        }
                        if(ownerObj.hasOwnProperty('Property Zip')){
                            delete ownerObj["Property Zip"];
                        }
                        let propertyObj: any = {...docToSave};
                        if(propertyObj.hasOwnProperty('Full Name')){
                            delete propertyObj["Full Name"];
                        }
                        if(propertyObj.hasOwnProperty('First Name')){
                            delete propertyObj["First Name"];
                        }
                        if(propertyObj.hasOwnProperty('Last Name')){
                            delete propertyObj["Last Name"];
                        }
                        if(propertyObj.hasOwnProperty('Name Suffix')){
                            delete propertyObj["Name Suffix"];
                        }
                        if (await self.civilAndLienSaveToNewSchema(ownerObj))
                            that.numberOfSavedRecords++;
                        if (await self.civilAndLienSaveToNewSchema(propertyObj))
                            that.numberOfSavedRecords++;
