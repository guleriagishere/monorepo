                ...
                for (let j = 0; j < valueForSelect.length; j++) {
                    ...
                    while (!flagStop) {
                        ...
                        for (let l = 0; l < totalRow!.length; l++) {
                            ...
                            let [codeViolationIdXpath] = await page.$x('//table[@id="ctl00_PlaceHolderMain_dgvPermitList_gdvPermitList"]/tbody/tr[contains(@class,"ACA_TabRow_Odd") or contains(@class,"ACA_TabRow_Even")][' + index + ']/td[2]');
                            let codeViolationId;
                            try {
