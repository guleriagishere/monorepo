...
export default class ForeclosureSource extends AbstractSource {
    ...
    async parseAndSave(): Promise<boolean> {
        ...
        if (!resultHandles.length) {
            ...
            if (resultHandles.length == 100 && addressesPreviouslyScraped < 10) {
                console.log('Crawling second page.')
                let page2url = this.parseObjFromStateString(this.stateToCrawl)?.url.replace('&ps=100&pg=1', '&ps=100&pg=2') as string;
                await this.browserPages?.generalInfoPage?.goto(page2url, { waitUntil: 'domcontentloaded' });
                resultHandles = await foreclosurePage.$x('//*[contains(@class, "listingRow")]');
                for (let resultHandle of resultHandles) {
            ...
            console.log(`${totalNoOfResults} new result(s) found.`)
            ...
            for (let county of Object.keys(countyPropsObj)) {
                ...
                for (let propertyAddressObj of countyPropsObj[county]) {
                    ...
                    lineItem['Property Address'] = propertyAddressObj['property_address'];
                    lineItem['Property City'] = propertyAddressObj['property_city'];
                    lineItem['County'] = propertyAddressObj['property_county'];
                    lineItem['Property State'] = propertyAddressObj['property_state'];
                    lineItem['Property Zip'] = propertyAddressObj['property_zip'];
                    lineItem.listedPrice = propertyAddressObj['listed_price'];
                    lineItem.listedPriceType = propertyAddressObj['listed_price_type'];
                    lineItem.practiceType = propertyAddressObj['practice_type'];
                    lineItem.productId = product._id;
                    lineItem.propertyAppraiserProcessed = false;
                    lineItem.vacancyProcessed = false;
                    await lineItem.save();
                    let data = {
                    ...
                    // try{
                    //     let property = await db.models.Property.findOne({ 'Property Address': propertyAddressObj['property_address'] });
                    //     if(!property){
                    //         let dataForProperty = {
                    //             'Property Address' : propertyAddressObj['property_address'],
                    //             'Property City' : propertyAddressObj['property_city'],
                    ...
                    //     const checkOwnerProductProperty = await db.models.OwnerProductProperty.findOne({ property: property._id, productId: product._id });
                    //     if (checkOwnerProductProperty) return true;
                    ...
                    //     }
                    //     let ownerProductProperty = new PublicRecordOwnerProductProperty(dataForOwnerProductProperty);
                ...
                //    const success: boolean = await sqsService.enqueue(SqsService.QUEUE_URL, [`/${this.publicRecordProducer.state}/${normalizedCountyNameForMongo}`], {
                //        'source': {
                //            DataType: 'String',
                //            StringValue: this.publicRecordProducer.source
                //        },
                //        'state': {
                //            DataType: 'String',
                //            StringValue: this.publicRecordProducer.state
                //        },
                //        'county': {
                //            DataType: 'String',
                //            StringValue: county
