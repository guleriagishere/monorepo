...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    async parseAndSave(docsToParse: IOwnerProductProperty[]): Promise<boolean>   {
        ...
        for (let i = start; i < docsToParse.length; i++) {
            ...
            if (this.searchBy === 'name') {
                ...
                try{
                    const parsed = parser.parseLocation(docsToParse[i].propertyId['Property Address']);
            ...
            if (rows.length > 0) {
                ...
                if (this.searchBy === 'name') {
                    for (let j = 0; j < rows.length; j++) {
                        ...
                        if (name_string?.includes(docsToParse[i].ownerId["Full Name"].replace(',', ''))) {
                            ...
                            if (linkHandle.length > 0) {
                                ...
                                let result;
                                try {
                                    result = await this.getPropertyInfos(detailPage, address, docsToParse[i].ownerId["Full Name"].replace(',', ''));
                                    if (!result) {
                ...
                let result;
                try {
                    result = await this.getPropertyInfos(page, address, this.searchBy === 'name' ? docsToParse[i].ownerId["Full Name"].replace(',', '') : '');
                    if (!result) {
                        continue;
                    }
                } catch (e) {
    ...
    async parseResult(result: any, document: any) {
        let dataFromPropertyAppraisers = {
            ...
            'Property State': result['property_address_state'],
