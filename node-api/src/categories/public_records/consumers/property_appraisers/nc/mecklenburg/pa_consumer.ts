...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    async parseResult(page: puppeteer.Page, link: any, document: any, address: any) {
    ...
    async parseAndSave(documentsToParse: any): Promise<boolean> {
        console.log(`documents to look up: ${documentsToParse.length}.`);

        const url_search = 'https://mnc.spatialest.com/#/';

        let countRecordsSuccess = 0;
        const page = this.browserPages.propertyAppraiserPage!;
        for (let document of documentsToParse) {
            ...
            if (this.searchBy === 'name') {
                ...
                try {
                    ...
                    if (address['parsed'] && address['parsed']['sec_unit_num']) {
                        search_value = address['street_with_type'] + ' #' + address['parsed']['sec_unit_num']
                    } else {
                        search_value = address['street_full'];
                    }
                } catch (e) {
                    search_value = document.propertyId['Property Address'];
                }
            ...
            if (flagMultipleRow) {
                try {
                    ...
                    for (let j = 0; j < rows.length; j++) {
                        ...
                        try {
                            await this.parseResult(page, href.link, document, address);
                        } catch (e) {
                        }
                    }
                } catch (err) {
                ...
                await this.parseResult(page, '', document, address);
            }
