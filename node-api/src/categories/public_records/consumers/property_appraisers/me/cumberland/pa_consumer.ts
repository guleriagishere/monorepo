...
//     urls = {
//         propertyAppraiserPage: 'https://assessors.portlandmaine.gov/search.htm'
//     }

//     xpaths = {
//         isPAloaded: '//input[@name="cmdsubmit"]'
//     }
...
//               try {
//                 // input street number
//                 const inputNumHandle = await page.$('input[name="hnumber"]');
//                 if (address['parsed']['number'] && inputNumHandle) {
//                   await inputNumHandle.type(address['parsed']['number'], {delay: 100});
//                 }
//                 // // input address
//                 const inputAddrHandle = await page.$('input[name="street"]');
//                 if (inputAddrHandle)
...
//                 // // check result
//                 const link_handle = await page.$('table#TABLE1 > tbody > tr:nth-child(2) > td:first-child > a');
//                 if (link_handle) {
//                   await Promise.all([
//                     link_handle.click(),
//                     page.waitForNavigation()
...
//       const name_addr_selector = 'form > table:first-of-type > tbody > tr:nth-child(4) > td:nth-child(2) > font';
//       const name_addr_str = await this.getElementHtmlContent(page, name_addr_selector);
//       const name_addr = name_addr_str.split("<br>").map(str => this.simplifyString(str));

//       // name
//       const owner_names = [];
//       const owner_name_arr = name_addr[0].split('&amp;').map(str => this.simplifyString(str));
...
//       // mailing address
//       const mailing_address = name_addr.slice(1).join(', ');
...
//       // property type
//       const property_type_selector = 'form > table:first-of-type > tbody > tr:nth-child(2) > td:nth-child(2) > font';
//       const property_type = await this.getElementTextContent(page, property_type_selector);

//       // sales info
//       const last_sale_date_selector = 'form > table:nth-of-type(5) > tbody > tr:nth-child(2) > td:first-child > font';
//       const last_sale_amount_selector = 'form > table:nth-of-type(5) > tbody > tr:nth-child(2) > td:nth-child(3) > font';
...
//       const total_assess_selector = 'form > table:nth-of-type(2) > tbody > tr > td:first-child > table > tbody > tr:nth-child(6) > td:nth-child(2) > font';
//       const total_assessed_value = await this.getElementTextContent(page, total_assess_selector);
//       const est_value = 'N/A';
