...
const effective_year_xpath = '//tr[./td[contains(., "Effective Year:")]]/td[2]/span';
...
export default class PAConsumer extends AbstractPAConsumer {
    ...
    getAddress(document: IProperty): any {
        ...
        const parsed = parseaddress.parseLocation(document['Property Address']);
            ...
            if (this.searchBy === 'name') {
                ...
                try {
                    ...
                    if (address['parsed'] && address['parsed']['sec_unit_num']) {
                        parser_address = address['street_with_type'] + ' #' + address['parsed']['sec_unit_num']
                    } else {
                        parser_address = address['street_full'];
                    }
                } catch (e) {
                    parser_address = doc.propertyId['Property Address'];
                }
    ...
    async parseResult(result: any, document: any) {
        ...
        for (const owner of ownersArray) {
            let dataFromPropertyAppraisers = {
                ...
                'Effective Year Built': result['effective_year'],
                'Est Equity': '',
                'Lien Amount': ''
            };
            try {
